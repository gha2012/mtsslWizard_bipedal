...
class Rotamer:	
	...
	def score(self, aa1, aa2, originalLabel, environmentAtomCoordinates, vdWcutOff, maxClash, internalClashCheck):
		...
		for chi in range (1, self.numberOfRotatingBonds + 1):
			...
			movingAtoms += translationVector
			if len(mWlabel.rotationInfo[str(chi)]) == 4: # this should only run when there is a "d" in the rotation string, i.e. when the bond length is supposed to be adjusted. This is needed for the Cu labels
		...
		# depending on the label select which atoms are included in the clash detection
		...
		if not mWlabel.quickClash(clashAtoms, environmentAtomCoordinates, vdWcutOff, maxClash):
			if not mWlabel.internalClash2(movingAtoms, refDist) or originalLabel.name == "nta" or originalLabel.name == "ida" or internalClashCheck == False:  # ignore internal clashes for nta or ida, adjusting the N-Cu bonds will screw the check up.
				# only update the main chain atoms now for the rmsd calculation. The remaining atoms are updated at the
				# very end. Faster this way.
				if originalLabel.name == "rx":
					trialAa2Array = numpy.array([movingAtoms[14], movingAtoms[15], movingAtoms[16], movingAtoms[13]])
				elif originalLabel.name == "ida":
					trialAa2Array = numpy.array([movingAtoms[13], movingAtoms[16], movingAtoms[15], movingAtoms[12]])
